        ...
        while True:
            ...
            for ind, word in enumerate(sentence.strip().split(' ')):
                ...
                if ind >= int(target_index_start) and ind < int(target_index_end):
                    ...
                    #o_masks.extend([int(ind)]*len(word_ids))
                else:
...
def load_and_preprocess_glosses(lemma_words, tokenizer, wn_senses, gloss_def, max_len=-1):
    sense_glosses = {}
    
    for target_key in lemma_words:
        if target_key not in sense_glosses:
            sensekey_arr = wn_senses[target_key]
            gloss_arr = [gloss_def[s] for s in sensekey_arr]
...
def load_and_preprocess_wn_glosses(lemma_words, tokenizer, wn_senses, max_len=-1):
    sense_glosses = {}
